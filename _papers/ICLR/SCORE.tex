...
\citep{chen2020automatic} proposes a pre-defined set of rhythmic groove patterns. This limits the model's ability to generate novel rhythms. 
...
divergence across pitch, beat, beat type, and expression similarity. The source code of this work is available at \url{https://github.com/DjentleViBe/SC0RE}.
...
An accent token is append to the note token from Section~\ref{sec:note} to indicate the presence of an expressive element. 
...
a BAR\textsuperscript{~\ref{app:secA1}}
...
during training follows the approach introduced in \citep{chunking}, 
...
\label{fig:riff}
\end{figure}
An example 16 tokens per sequence embedding is as follows: 
\texttt{[27049, 7985, 11849, 11849, 7985, 11849, 11849, 7985, 11849, 11849, 7985, 11849, 11849, 27049, 17485, 17506]}

\texttt{[11849, 11849,  7985, 11849, 11849, 27049, 17485, 17506, 27051,  7802, 7802, 11848, 11848, 15530, 27074, 27049]}

\texttt{[27051,  7802,  7802, 11848, 11848, 15530, 27074, 27049, 27075, 0, 0, 0, 0, 0, 0, 0]}
...
The guitar tracks were obtained from \citep{SCORESET}.
...
The common practice of 80/20 split was used to create the training and validation sets \citep{Goodfellow2016DeepLearning}. 
...
Relavant statistics about the dataset used for training and evaluation are provided in \citep{begari2025scoresetdatasetguitarprofiles}.
\begin{figure}[h]
\centering
\begin{minipage}{0.48\linewidth}
\includegraphics[clip, width=0.9\linewidth]{trainingaccentprobability.pdf}
\caption{Distribution of accent tokens in the training data after random sampling, highlighting common expressive elements that the model learns to reproduce.}
  \label{fig:accentnote}
\end{minipage}\hfill
\begin{minipage}{0.48\linewidth}
\includegraphics[clip, width=0.9\linewidth]{startprobability.pdf}
\caption{Distribution of starting notes for musical measures, used to select initial tokens for more consistent generation.}
  \label{fig:firstnote}
\end{minipage}
\end{figure}
...
attention as described in \citep{Vaswani2}. Although, segment-level 
...
A simpler version of the attention matrix from \citep{XL} is used from which the KL divergence-based sequence bias is subtracted:
\begin{align}
...
A variant of the repetition penalty introduced in \citep{reppenalty} is employed; 
however, instead of greedy decoding, multinomial sampling is used to generate 
tokens to get semantically different output sequences \citep{song2024}. 
Including this loss during training removes the need for explicit constraints during 
inference for generating valid sequences.
\begin{align}
...
denote the hidden-state tensor produced by the model for a batch of \(B\) sequences, where \(H\) is the number of attention heads, \(T\) is the sequence length, and \(d\) is the per-head hidden dimensionality. 
The heads are first merged into a single model embedding of dimension \(D = H \cdot d\), then mean-pool is taken over the temporal dimension. 
A per-sequence memory vector \(\mathbf{m} \in \mathbb{R}^{B \times D}\) is added to obtain the final sequence embedding.

\begin{align}
\tilde{\mathbf{H}}_b &= \frac{1}{T} \sum_{t=1}^{T} 
\operatorname{reshape}\big(\mathbf{H}_{b,:,t,:}\big) 
\in \mathbb{R}^{D}, \quad b = 1, \dots, B \\
\mathbf{E}_b &= \tilde{\mathbf{H}}_b + \mathbf{m}_b 
\in \mathbb{R}^{D}, \quad b = 1, \dots, B
\end{align}

where \(\mathbf{E} \in \mathbb{R}^{B \times D}\) is the batch of final sequence embeddings.
To encourage structural consistency of the embedding space across successive 
training steps, a relative distance preservation loss is introduced. 
Let $\mathbf{E}^{(t)}$ and $\mathbf{E}^{(t-1)}$ denote the sequence 
embeddings at the current and previous steps, respectively. For each batch, 
the pairwise squared Euclidean distance matrices \cite{Dokmanic_2015} is computed:
\begin{equation}
\mathbf{D}^{(t)}_{ij}
=
\left\lVert
\mathbf{E}^{(t)}_i - \mathbf{E}^{(t)}_j
\right\rVert_2^2,
\quad
\mathbf{D}^{(t-1)}_{ij}
=
\left\lVert
\mathbf{E}^{(t-1)}_i - \mathbf{E}^{(t-1)}_j
\right\rVert_2^2,
\end{equation}
for all $i,j \in \{1,\dots,B\}$.

The regularization term is an implementation of relational distance preservation objectives 
used in similarity preserving knowledge distillation \cite{tung2019similaritypreservingknowledgedistillation}. 
Unlike prior teacher-student formulations, the model's previous embedding state is used as a fixed reference, 
enforcing temporal consistency via a detached pairwise distance target.
The relative distance regularization term is defined as:
\begin{equation}
...
\right)^2,
\end{equation}
where $\delta$ is a scalar weighting 
coefficient and $\operatorname{stopgrad}(\cdot)$ denotes 
gradient detachment, making it a fixed reference geometry.

This loss constrains the evolution of the embedding 
space by preserving pairwise relationships between samples, 
rather than enforcing absolute alignment of embeddings. 
As a result, it promotes temporal stability and mitigates 
representational drift while allowing global transformations 
of the embedding space.

\section{Results}\label{sec2}
The idea of context carryover has been well established in \citep{llmgate} \citep{wei2021attentivecontextualcarryovermultiturn} \citep{pmlr-v267-chen25cg}
\citep{rae2019compressivetransformerslongrangesequence} \citep{beltagy2020longformerlongdocumenttransformer} \citep{zaheer2021bigbirdtransformerslonger}. 
...
subsequent measure, following the approach described in \citep{remi}.
...
\citep{Seo2016} \citep{zhang2025aestheticshumanpreferencescomparative} \citep{spiliopoulou2011comparingprobabilisticmodelsmelodic}:
\begin{align}
...
\begin{figure}[H]
  ...
  \caption{Loss plots over 200 epochs for a median of 3 independent training runs for hyper-parameter values listed in Table~\ref{tab:hyperparam}}
\label{fig:lossplots}
\end{figure}
...
Figures~\ref{fig:noteskl}-\ref{fig:beattypekl} visualize the results.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\linewidth]{3_test_all_T_compareprobabilitynotes.pdf}
  \caption{KL divergence between generated and true distributions of musical notes, indicating accurate note prediction by the model.}
\label{fig:noteskl}
\end{figure}
...
\begin{minipage}{0.48\linewidth}
  \centering
  \includegraphics[width=1.0\linewidth]{3_test_all_T_compareprobabilitybeats.pdf}
  \caption{KL divergence for beat tokens with start token 7729, showing the model preserves rhythmic structure.}
\label{fig:beatskl}
\end{minipage}\hfill
\begin{minipage}{0.48\linewidth}
  \centering
  \includegraphics[width=1.0\linewidth]{3_test_all_T_compareprobabilitybeattype.pdf}
  \caption{KL divergence for beat types with start token 7729, demonstrating correct modeling of musical subdivisions.}
...
The ranges for the grid search are given in the repository \url{https://github.com/DjentleViBe/SC0RE}.
