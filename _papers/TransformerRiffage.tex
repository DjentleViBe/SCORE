...
subdivided into 14 distinct rhythmic values which is listed in Appendix~\ref{app:B}, which makes a total to 82 unique beat indices.
...
Relavant statistics about the dataset used for training and evaluation are provided in Appendix~\eqref{app:C}
\section{Inference}
...
\begin{figure}[H]
  \centering
\includegraphics[clip, width=0.6\linewidth]{dec_RPE_ann_CB_Large_startprobability.pdf}
...
\begin{figure}[H]
  \centering
\includegraphics[clip, width=0.6\linewidth]{dec_RPE_ann_CB_Large_trainingaccentprobability.pdf}
\caption{Accent occurrences in training data.}
  \label{fig:note}
\end{figure}
...
Loss values observed after 100 epochs of training:\\
\begin{table}[htbp]
 \centering
\begin{tabular}{cc}
\hline
  \multicolumn{1}{|c|}{\textbf{Loss}} & \multicolumn{1}{|c|}{\textbf{Value}}\\
  \hline
  \multicolumn{1}{|c|}{Cross Entropy} & \multicolumn{1}{|c|}{1.4629}\\
  \hline
  \multicolumn{1}{|c|}{Repetition} & \multicolumn{1}{|c|}{0.1368}\\
  \hline
  \multicolumn{1}{|c|}{Sequence} & \multicolumn{1}{|c|}{0.1985}\\
  \hline
  \multicolumn{1}{|c|}{Total} & \multicolumn{1}{|c|}{1.7983}\\
  \hline
  \multicolumn{1}{|c|}{Validation} & \multicolumn{1}{|c|}{2.0622}\\
    \hline
\end{tabular}
\captionsetup{type=table}
\caption{Loss values}
...
\begin{figure}[H]
  \centering
\includegraphics[clip, width=0.8\linewidth]{dec_RPE_ann_CB_Large_L_compareprobabilitynotes.pdf}
\includegraphics[clip, width=0.8\linewidth]{dec_RPE_ann_CB_Large_L_compareprobabilitybeats.pdf}
\caption{Note and beat occurrences of training data and test data}
  \label{fig:note}
\end{figure}


The model produces musically pleasing sequences, effectively combining notes, beats, and accents while maintaining sequence context between measures.
\begin{figure}[H]
  \centering
\includegraphics[trim={20 200 0 130}, clip, width=0.8\linewidth]{dec_RPE_ann_CB_Large_L_inference.pdf}
\caption{Test results showing 4 musical measures}
  \label{fig:inference}
\end{figure}
...
The rhythm guitar tracks were extracted manually from the original Guitar Pro files. Finger adjustments present in the source files are described in detail in the encoding section. No additional normalization or filtering—such as pitch standardization or sequence length filtering—was performed beyond this manual selection.

The dataset consists exclusively of rhythm guitar tracks in .gp5 format with a custom tuning (F C G\# D\# A\# D\#). It contains 6,208 sequences, each with a maximum length of 32 tokens.

An 80/20 split was used to create the training and validation sets. For testing, the top 10 first notes of each measure were selected as prompts for sequence generation and evaluation.

Due to copyright restrictions, the original .gp5 files and preprocessing scripts cannot be shared. However, the code and model pipeline are designed to operate on any Guitar Pro dataset, provided the files are preprocessed into the expected format as described.
